...
| File | Role |
|---|---|
| `graph.py` | `create_quant_agent(config, checkpointer, approval_callback=None)` — the only public API for building the agent. Wiring order: (1) `_create_chat_model(config)` — parses `config.model` as `"provider:model"`; if the provider is `zai` or `opencode`, it calls `init_chat_model(model=name, model_provider="openai", api_key=..., base_url=...)` (an OpenAI-compatible gateway, not a native LangChain provider); otherwise it calls `init_chat_model(name, model_provider=provider)` for native providers (anthropic, openai, google_genai, openrouter). (2) `build_tool_registry(config)`. (3) `SkillResolver(...).resolve()`. (4) main `FilesystemBackend(root_dir=~/.quantagent, virtual_mode=False)` — the agent's own read/write workspace. (5) `_stage_resolved_skills(resolved.skill_dirs)` — deepagents' `SkillsMiddleware` expects each `sources` entry to be a directory *containing* skill subdirectories, not an individual skill's own directory, so the resolved individual skill folders are symlinked into one rebuilt-per-call staging dir (`~/.quantagent/.resolved-skills/`) that matches that shape; `SkillsMiddleware(backend=FilesystemBackend(virtual_mode=False), sources=[staging_dir])` is built explicitly (on its own backend, decoupled from the main workspace backend) rather than via `create_deep_agent(skills=...)`. (6) middleware list, in order: `TodoListMiddleware()` (not auto-injected by `create_deep_agent` since deepagents 0.7.0), the `SkillsMiddleware` from step 5, `ErrorLoggingMiddleware()`, `ToolProgressMiddleware()`, `SummarizationMiddleware(token_threshold=80_000, model=model)`, then `ApprovalMiddleware(...)` appended only if `config.approval_required` is non-empty. (7) `create_deep_agent(...)` with the main `backend`, `checkpointer`, `middleware`. |
...
| `backtesting.py` | `run_backtest` (vectorbt-based), `run_walkforward` (optionally re-optimizes each fold's parameters via a `param_grid`, exposing the winner as `BacktestResult.best_params`), `optimize_parameters` (grid search, delegates to `_grid_search`), `format_backtest_result`. Returns a frozen `BacktestResult` pydantic model. |
...
13 built-in skill directories at the **repo root** `skills/` (not inside `quantagent/`), each a directory with a `SKILL.md` (YAML frontmatter: `name`, `description`, `license: MIT`, `metadata.author`/`version`, `allowed-tools`) plus optional reference files. deepagents reads only `description` at startup to decide relevance; the full body loads on demand when a user prompt matches.
